...
nomes = ["Ana", "Bruno", "Carlos", "Diana", "Eduardo", "Filipa","Joana","Manuel","Francisco","Margarida"]
apelidos = ["Silva", "Santos", "Oliveira", "Pereira", "Costa", "Fernandes","Alves","Marques","Aguiar","Almeida","Rocha","Pinho","Figueiredo","Lencastre"]
...
def setup():
    ...
    if option == 0:  # Opção 1 (weekend)
        hlist = random.sample(horas_disponiveis, 3)  # Pick 3 random hours
...
def contexto_previo():
    opcoes = [
        ...
        "Agente - Estamos a contacta-lo no seguimento de um formulário que prencheu através dos nossos canais digitais. Gostaria de o convidar a realizar uma consulta de avaliação dentária, sem qualquer tipo de custo, na nossa {clinica} da Smileup {local} , deseja marcar esta consulta?\n Cliente - Pode ser."
    ]
    return random.choice(opcoes)
...
def marcar_para_familiar_ou_amigo():
    ...
    return random.choice(opcoes)
# 10 - Confirmação
def confirmacao(day, dia, hour, clinic_name):
    opcoes = [
        f"Agente - Então a consulta fica marcada para {day}, dia {monthday} às {hour} na {clinic_name}. Irá receber um e-mail com as informações da consulta e um SMS no dia anterior. Tem alguma questão ou posso lhe ajudar com mais alguma coisa?\nCliente - Não, obrigado.",
        f"Agente - A sua consulta ficou então agendada para {day}, dia {monthday} às {hour} na {clinic_name}. Depois receberá um email com os detalhes da consulta e vai receber um SMS no dia antes da consulta. Se tiver mais alguma questão ou se puder ajudar-lhe com mais alguma coisa...\nCliente - Não. É tudo.",
        f"Agente - Fica então para, {day}, dia {monthday} às {hour} na {clinic_name}, agendada a sua consulta. Ok {nome_cliente}? Tem mais alguma questão?\nCliente - Não, era tudo, obrigado."
    ]
    return random.choice(opcoes)
# 11 - Finalização
def finalizacao():
...
def simulacao():
    ...
    setup()
    if direcao_chamada == "cliente_para_agente":
        print(introducao_cliente_para_agente())
    else:
        print(introducao_agente_para_cliente())
        print(contexto_previo())
    print(pedido_gravacao())
    ...
    if direcao_chamada == "cliente_para_agente":
        ...
        print(identificar_clinica_agente_para_cliente())
    else:
        print(solicitacao_de_dados_agente_para_cliente())
